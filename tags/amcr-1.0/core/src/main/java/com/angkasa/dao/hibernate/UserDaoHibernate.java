...
public class UserDaoHibernate extends GenericDaoHibernate<User, Long> implements UserDao, UserDetailsService {
    ...
    public User getUsersByCoop(Long coopId) {
        if(coopId != null){
            Query qry = getSession().createQuery("from User u where coopId=" + coopId + " order by upper(u.username)");

            List<User> result = qry.list();
            if(!result.isEmpty()){
                return result.get(0);
            }
        }
    ...
    public User getUsersByMember(Long memberId) {
        if(memberId != null){
            Query qry = getSession().createQuery("from User u where memberId=" + memberId + " order by upper(u.username)");
            List<User> result = qry.list();
            if(!result.isEmpty()){
                return result.get(0);
            }
    ...
    public User getByUsername(String username) {
        if(StringUtils.isNotBlank(username)){
            Query qry = getSession().createQuery("from User u where username='" + username + "' order by upper(u.username)");
            List<User> result = qry.list();
            if(!result.isEmpty()){
                return result.get(0);
            }
    ...
    public User getByEmail(String email) {
        if(StringUtils.isNotBlank(email)){
            Query qry = getSession().createQuery("from User u where email='" + email + "' order by upper(u.username)");
            List<User> result = qry.list();
            if(!result.isEmpty()){
                return result.get(0);
            }
