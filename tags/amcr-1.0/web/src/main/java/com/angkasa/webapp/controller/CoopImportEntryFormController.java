...
public class CoopImportEntryFormController extends BaseFormController {
    ...
    @RequestMapping(value = "/{coopImportId}/{importId}", method = RequestMethod.GET)
    public ModelAndView showEntry(@PathVariable("coopImportId") String coopImportId, @PathVariable("importId") String importId, HttpServletResponse response) throws Exception {
    ...
    public String onSubmitEntry(Coop coop, BindingResult errors, HttpServletRequest request, HttpServletResponse response,RedirectAttributes redirectAttributes)
            throws Exception {
        redirectAttributes.addFlashAttribute("id",coop.getCoopImportId());
