...
public class PropsUtil implements Serializable {
    ...
    public Map<String, String> getTransactionTypeList(){
        Map<String, String> list = new HashMap<>();
        list.put(Constants.TRANS_TYPE_DEDUCT_ACCT, Constants.TRANS_TYPE_DEDUCT_ACCT);
        list.put(Constants.TRANS_TYPE_DEDUCT_SALARY, Constants.TRANS_TYPE_DEDUCT_SALARY);
        list.put(Constants.TRANS_TYPE_DEPOSIT_ACCT, Constants.TRANS_TYPE_DEPOSIT_ACCT);

        return list;
    }

    public Map<String, String> getMemberImportCoopMatchList(){
        Map<String, String> list = new HashMap<>();
        list.put(MemberImport.COOP_MATCH_COOPCODE, MemberImport.COOP_MATCH_COOPCODE);
        list.put(MemberImport.COOP_MATCH_BPACODE, MemberImport.COOP_MATCH_BPACODE);
        list.put(MemberImport.COOP_MATCH_SKMNO, MemberImport.COOP_MATCH_SKMNO);
    ...
    public Map<String, String> getProductPeriodList(){
        Map<String, String> list = new HashMap<>();
        list.put(Product.PERIOD_DAYS, Product.PERIOD_DAYS);
        list.put(Product.PERIOD_MONTHLY, Product.PERIOD_MONTHLY);
    ...
    public boolean isUseCoopCodeInMemberImport(){
        String value = systemSettingManager.valueByPropertyName(Constants.SYSTEM_SETTING_USE_COOP_CODE_IN_MEMBER_IMPORT);
        if(StringUtils.equalsIgnoreCase(value, "true")){
            return true;
        }
        return false;
    ...
    public boolean isEnableDmsApi(){
        String value = systemSettingManager.valueByPropertyName(Constants.SYSTEM_SETTING_ENABLE_DMS_API);
        if(StringUtils.equalsIgnoreCase(value, "true")){
            return true;
        }
        return false;
