...
public class MemberImportDaoHibernate extends GenericDaoHibernate<MemberImport, Long> implements MemberImportDao {
    ...
    @Override
    public MemberImport saveWithFile(MemberImport memberImport, boolean useCoopCode) throws Exception{
...
//                List<Member> memberListAfter = memberDao.validateMemberImportList(memberList, useCoopCode);
                List<Member> memberListAfter = memberDao.processMemberImport(memberList, useCoopCode);
                ...
                for(Member mem : memberListAfter){
                    ...
                    if(mem.getImportStatus().equals(Constants.IMPORT_STATUS_FAILED) || mem.getImportStatus().equals(Constants.IMPORT_STATUS_INVALID)){
                        hasFailed = true;
                    }
