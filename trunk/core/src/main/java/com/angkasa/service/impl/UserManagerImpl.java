...
public class UserManagerImpl extends GenericManagerImpl<User, Long> implements UserManager, UserService {
    ...
    @Override
    public User saveUser(final User user, boolean updateMember, boolean updateCoop) throws UserExistsException, MemberNotFoundException, CoopNotFoundException, MemberAlreadyHasUserException, CoopAlreadyHasUserException {
        if (updateMember) {
        ...
        if (updateMember) {
            Member member = memberDao.getByIcNumber(user.getIcNumber());
            if(member.getUserId() != null && member.getUserId()!=user.getId()){
                log.warn("Member already has user!");
                throw new MemberAlreadyHasUserException("Member already has user!");
            }
        ...
        if (updateCoop) {
            Coop coop = coopDao.getByCoopCode(user.getCoopCode());
            if(coop.getUserId() != null && coop.getUserId()!=user.getId()){
                log.warn("Coop already has user!");
                throw new CoopAlreadyHasUserException("Coop already has user!");
            }
    ...
    public User updatePassword(final String username, final String currentPassword, final String recoveryToken, final String newPassword, final String applicationUrl) throws UserExistsException {
        return updatePassword(username, currentPassword, recoveryToken, newPassword,applicationUrl, null);
    }
     ...
     */
    @Override
    public User updatePassword(final String username, final String currentPassword, final String recoveryToken, final String newPassword, final String applicationUrl, User requestingUser) throws UserExistsException {
        return updatePassword(username,currentPassword,recoveryToken,newPassword,applicationUrl,requestingUser, true);
    }

    @Override
    public User updatePassword(final String username, final String currentPassword, final String recoveryToken, final String newPassword, final String applicationUrl, User requestingUser, boolean sendMail) throws UserExistsException {
        ...
        if (isRecoveryTokenValid(user, recoveryToken)) {
            ...
            return user;
        } else if (requestingUser != null && requestingUser.isAdminUser()) {
            log.debug("Updating password (admin override) for user:" + username);
            user.setPassword(newPassword);
            user = saveUser(user);
            return user;
    ...
    @Override
    public User getByHexLogin(String username, String password) {
        User user = getByUsername(username);
        if (user != null) {
            if (passwordEncoder.matches(password, user.getPassword())) {
                return user;
