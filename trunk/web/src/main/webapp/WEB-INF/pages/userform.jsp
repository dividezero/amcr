...
<div class="col-sm-7">
    ...
    <form:form commandName="user" method="post" action="userform" id="userForm" autocomplete="off"
        ...
        <div class="form-group${(not empty status.errorMessage) ? ' has-error' : ''}">
            ...
            <c:if test="${pageContext.request.remoteUser == user.username || currentUser.adminUser}">
                <span class="help-block">
                    <a href="<c:url value="/updatePassword?username=${user.username}" />"><fmt:message key='updatePassword.changePasswordLink'/></a>
                </span>
            </c:if>
    ...
    <c:when test="${param.from == 'list' or param.method == 'Add'}">
        ...
        <div class="form-group">
            ...
            <select id="userRoles" name="userRoles" multiple="true" class="form-control">
                <c:choose>
                    <c:when test="${user.coopUser || user.memberUser}">
                        <c:set var="rolesShown" value="${availableRoles}"/>
                    </c:when>
                    <c:otherwise>
                        <c:set var="rolesShown" value="${rolesList}"/>
                    </c:otherwise>
                </c:choose>

                <c:forEach items="${rolesShown}" var="role">
                <option value="${role.value}" ${fn:contains(user.roles, role.label) ? 'selected' : ''}>${role.label}</option>
                </c:forEach>
            </select>
            <div id="icNumberDiv" name="icNumberDiv">
                <br/>
                <label for="icNumber" class="control-label"><fmt:message key="userProfile.member"/></label>
            ...
            <div id="coopCodeDiv" name="coopCodeDiv">
                <br/>
                <label for="coopCode" class="control-label"><fmt:message key="userProfile.coop"/></label>
                <%--<form:input cssClass="form-control" path="coopCode" id="coopCode"/>--%>
                <form:select cssClass="form-control" path="coopCode" items="${coopList}" disabled="true"/>
                <form:hidden path="coopCode"/>
            </div>
        </div>
